var FUNCTIONSERVICE = {
	...
	whoIsSmartMirror : function($scope) {
		...
		if(responsiveVoice.voiceSupport()) {
	          responsiveVoice.speak("저는 음성 인식이 가능한 거울아 거울아 입니다.","Korean Female");
        }
	...
	wake : function($scope) {
		...
		if(responsiveVoice.voiceSupport()) {
            responsiveVoice.speak("안녕하세요. 거울아거울아 예요!","Korean Female");
          }
    	$scope.focus = "default";
	},
		...
		SubwayService.init(station).then(function(){
            SubwayService.getArriveTime(linenumber,updown).then(function(data){
              if(data != null){
                ...
                if(responsiveVoice.voiceSupport()) {
                	responsiveVoice.speak(data[1].ARRIVETIME + "에 " + data[1].SUBWAYNAME + "행 열차가 있습니다. 이어서,"+data[2].ARRIVETIME + "에 " + data[2].SUBWAYNAME + "행 열차가 있습니다.","Korean Male");
                }
              }else{
	...
	photo : function(PHOTO_INDEX) {
		...
		if(responsiveVoice.voiceSupport()) {
			responsiveVoice.speak("촬영 시작","Korean Female");
		}
	...
	lightOn : function() {
    	...
    	if(responsiveVoice.voiceSupport()) {
            responsiveVoice.speak("등을 켭니다.","Korean Male");
        }
	...
	lightOff : function() {
		...
		if(responsiveVoice.voiceSupport()) {
            responsiveVoice.speak("등을 끕니다.","Korean Male");
        }
